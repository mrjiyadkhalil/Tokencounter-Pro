...
const App: React.FC = () => {
  ...
  const handleSimulateLoad = async () => {
    ...
    try {
      ...
      const prompt = `Generate a long and detailed technical report about the scaling laws of large language models, specifically focusing on context windows up to 1 million tokens. Be verbose and informative.`;
      ...
      alert(error instanceof Error ? error.message : 'Generation error. Please check your connection.');
    } finally {
    ...
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-900 leading-none">TokenScale Pro</h1>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">Context Analysis Engine</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            {activeTab === 'calculator' ? (
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-[calc(100vh-220px)] min-h-[550px]">
                <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                  <div className="flex items-center space-x-3">
                    <label htmlFor="model-select" className="text-xs font-black text-slate-500 uppercase tracking-widest">Model Engine</label>
                    <select 
                      id="model-select"
                      value={selectedModel}
                      onChange={(e) => setSelectedModel(e.target.value as ModelType)}
                      className="bg-white border border-slate-300 text-slate-900 text-sm font-bold rounded-lg focus:ring-2 focus:ring-blue-500 block px-3 py-1.5 outline-none cursor-pointer hover:border-slate-400 transition-all"
                    >
                ...
                <div className="flex-grow relative bg-white">
                  <textarea
                    className="absolute inset-0 w-full h-full p-8 text-slate-900 bg-white leading-relaxed focus:outline-none resize-none mono text-base placeholder-slate-400 font-medium"
                ...
                <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-slate-400">
                  <div className="flex space-x-8">
                    <span>Characters: <span className="text-slate-900 font-mono font-bold">{formatNumber(stats.characters)}</span></span>
                    <span>Words: <span className="text-slate-900 font-mono font-bold">{formatNumber(stats.words)}</span></span>
                  </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gradient-to-br from-slate-900 to-blue-900 p-8 rounded-2xl text-white shadow-2xl border border-white/10">
                  <h3 className="text-2xl font-black mb-3 tracking-tight">The Million-Token Horizon</h3>
                  <p className="text-slate-300 mb-8 leading-relaxed font-medium">
                    Models have evolved past traditional constraints. 1 million tokens enables massive data ingest, 
                    full codebase reasoning, and deep context persistence.
                  </p>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                    <div className="bg-white/5 p-5 rounded-2xl backdrop-blur-md border border-white/10">
                      ...
                      <div className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">Context Window</div>
                    </div>
                    <div className="bg-white/5 p-5 rounded-2xl backdrop-blur-md border border-white/10">
                      <div className="text-3xl font-black text-indigo-400 mb-1">12k+</div>
                      <div className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">Code Files</div>
                    </div>
                    <div className="bg-white/5 p-5 rounded-2xl backdrop-blur-md border border-white/10">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
              <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-8">Token Analytics</h2>
              ...
              <div className="space-y-10">
                <div>
                  ...
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-widest">Estimated Tokens</div>
                </div>
              ...
              <button 
                ...
                className={`w-full mt-10 py-4 px-6 rounded-xl font-black uppercase tracking-widest text-xs flex items-center justify-center space-x-3 transition-all shadow-xl active:scale-[0.97] ${isSimulating ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-slate-900 text-white hover:bg-blue-600'}`}
              >
                {isSimulating ? (
                  <>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Session History</h2>
                {history.length > 0 && (
              ...
              {history.length === 0 ? (
                <div className="py-8 text-center text-slate-300 font-bold text-xs uppercase tracking-widest">Empty Log</div>
              ) : (
                <div className="space-y-4 max-h-[300px] overflow-y-auto pr-1">
                  {history.map((item) => (
                    <div key={item.id} className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                      ...
                      <p className="text-[11px] text-slate-600 line-clamp-2 font-medium mb-2 italic">"{item.text}"</p>
      ...
      <footer className="bg-white border-t border-slate-200 py-10 mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-[11px] font-bold text-slate-400 tracking-widest uppercase">TokenScale Pro &copy; {new Date().getFullYear()} &bull; Professional context planning for Gemini, GPT, and Claude.</p>
        </div>
      </footer>
    </div>
  );
